...
const GatekeeperStream = ({ isDarkMode }) => {
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("");
  const [selectedPR, setSelectedPR] = useState(null);
  const observerTarget = useRef(null);

  const { feed, loading, hasMore, loadMore } = useGatekeeperFeed(false, {
    status: statusFilter,
    search: searchTerm,
  });
          ...
          {loading && feed.length === 0 ? (
            ...
            feed.map((pr, index) => (
              <div
                ...
                className={`border-l-4 ${getStatusColor(pr.status)} rounded p-4 transition-all cursor-pointer hover:shadow-lg ${
                  isDarkMode
              ...
              >
                  ...
                  <div className="text-right flex-shrink-0 bg-gray-50 dark:bg-slate-800 p-2 rounded">
                    <div
                      className={`text-2xl font-bold ${
                        pr.healthScore?.current < 50
                          ? "text-red-500"
                          : pr.healthScore?.current < 80
                            ? "text-yellow-500"
                            : "text-green-500"
                      }`}
                    >
                ...
                <div className="flex gap-2 text-xs mt-2 flex-wrap">
                  ...
                  <span
                    className={`px-2 py-1 rounded ${
                      pr.analysisResults?.aiScan?.verdict === "BAD"
                        ? "bg-red-200 text-red-900"
                        : "bg-green-200 text-green-900"
                    }`}
                  >
                ...
                {pr.analysisResults?.aiScan?.findings?.length > 0 && (
                  <div
                    className={`mt-3 p-2 text-xs rounded border flex gap-2 ${
                      isDarkMode
